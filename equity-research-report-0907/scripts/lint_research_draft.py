...
def paragraph_shape_warnings(text: str) -> list[dict]:
    """Spot repeated formatting artifacts without requiring every paragraph to be bold."""
    warnings = []
    # Ignore code examples and internal notes; neither is report body prose.
    ...
    excluded = re.compile(
        r"^(?:#{1,6}\s|\||>\s|[-*+]\s|[0-9]+[.)、]\s*|[①②③④⑤]|"
        r"(?:图表|图|表)\s*(?:[FTI]\d+|\d+)\s*[：:]|"
        r"(?:资料来源|数据来源|来源|注|报告类型|数据截止日|估值基准日|报告币种)[：:])"
    )
    for index, block in enumerate(blocks):
        if excluded.match(block):
            short_run = []
            continue
        if re.match(r"^\*\*[^*。！？\n]{1,12}[：:]\*\*\s*\S", block):
            label_only.append(block)
        if (
            re.fullmatch(r"\*\*[^*\n]{8,100}[。！？]\*\*", block)
            and index + 1 < len(blocks)
            and not excluded.match(blocks[index + 1])
            and not blocks[index + 1].startswith("**")
            and len(blocks[index + 1]) >= 40
        ):
            detached.append(block)
        plain = re.sub(r"\*\*|__", "", block)
        if 15 <= len(plain) <= 90 and len(re.findall(r"[。！？]", plain)) == 1:
            short_run.append(plain)
            if len(short_run) == 4:
                fragmented.append(" / ".join(short_run))
        else:
            short_run = []
    if len(label_only) >= 3:
        warnings.append(issue(
            "warning", "LABEL_ONLY_BOLD",
            "多处仅加粗短维度标签；普通分析段优先用完整概括句领起。预测假设等固定格式可保留。",
            label_only[0][:180],
        ))
    if len(detached) >= 3:
        warnings.append(issue(
            "warning", "DETACHED_BOLD_LEAD",
            "多处加粗句与解释分为两个段落；若为正文领句，应与解释同段。真实小节标题可保留。",
            detached[0],
        ))
    if fragmented:
        warnings.append(issue(
            "warning", "FRAGMENTED_PROSE",
            "发现至少4个连续单句短段；检查是否为同一问题被拆碎。独立事件、摘要或风险条目可保留。",
            fragmented[0][:240],
        ))
    return warnings
...
def style_warnings(text: str) -> list[dict]:
    ...
    warnings = paragraph_shape_warnings(text)
    ...
    if long_paragraphs:
        warnings.append(
            issue(
                "warning",
                "LONG_PARAGRAPH",
                f"发现 {len(long_paragraphs)} 个超过 500 字的正文段落；检查是否跨越多个中心问题。信息密集但主题统一时可保留，不按字数强拆。",
    ...
    if long_sentences:
        warnings.append(
            issue(
                "warning",
                "LONG_SENTENCE",
                f"发现 {len(long_sentences)} 个超过 120 字的句段；检查主干、嵌套和话题切换。同一因果链清楚时可保留，不按长度强拆。",
    ...
    if judgment_count >= 5 and density > 3:
        warnings.append(
            issue(
                "warning",
                "JUDGMENT_OVERUSE",
                f"‘我们认为/判断/预计’使用偏密（{density:.1f} 次/千字）；检查是否重复标记同一判断；保留来源归属与必要的预测标记，不为降低词频删去边界。",
            )
        )
    ...
    if repetitive_openers:
        ...
        warnings.append(
            issue(
                "warning",
                "REPETITIVE_OPENERS",
                f"至少三个相邻段落均以‘{opener}’起笔；改用对象、时间、数据或完整概括句切入。",
                evidence[:180],
            )
        )
